...
export class ExamTableComponent {
  ...
  data_length: string = localStorage.getItem('add_exam_details')
    ? JSON.parse(localStorage.getItem('add_exam_details') || '[]').length
    : 0;
  ...
  deleteItem(index: number) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        localStorage.setItem('add_exam_details', JSON.stringify(this.people));
